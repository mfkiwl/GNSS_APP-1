...
##########################################################################
#@function: to calcualte azimuth and elevation from satellite and 
#             reciever coordinates in ECEF
#@param:    x, y, z: satellite coodinates
#           xr, yr, zr: reciever coodinates
#           R_p, R_0: semi=mojor and semi-minor axis of elipsoid
#           f ....
#@return:   cbind(azimuth, elevation)  
#@usage: source("calculate_azimuth_elevation.R")
###########################################################################
calculate_azimuth_elevation <- function(x,y,z, xr, yr, zr, R_0 = 6378137, R_p = 6356752.314245179497563967, f=1/298.257223563){
  ...
  azimuth <- (bearing(c(coord_sphere_rec[1],coord_sphere_rec[2]),c(coord_sphere_sat[1],coord_sphere_sat[2])))
  ...
  pos_R_geo_cur <- cbind(pos_R_geo[1]*180/pi,pos_R_geo[2]*180/pi,pos_R_geo[3]) # = result
  
  pos_R_xyz_cur <- geo2pos(pos_R_geo_cur) # = c(xr,yr,zr)
  pos_R_0_xyz <- geo2pos((pos_R_geo_cur+c(rep(0,dim(pos_R_geo_cur)[1]),rep(0,dim(pos_R_geo_cur)[1]),rep(-5,dim(pos_R_geo_cur)[1]))))
  ...
  return(cbind("azimuth"=azimuth, "elevation"=elevation, "distance"=distance))
}
